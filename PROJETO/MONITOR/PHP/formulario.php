...
if(isset($_POST['submit'])) {
    ...
    if(!empty($texto)) {
        ...
        if($result) {
            echo "<div id='mensagem'>Texto cadastrado com sucesso!</div>";        
            echo "<script>setTimeout(function() { window.location.href = '../HTML/perguntas.php'; }, 3000);</script>";
        } else {
            echo "<div id='mensagem-erro'>Erro ao cadastrar o texto no banco de dados.</div>";
        }
    } else {
        echo "<div id='mensagem-erro'>O campo de texto deve ser preenchido.</div>";
    }
}
?>
